...
async function getAiUsageMeta(userId, baseCost) {
  const vipSnapshot = await syncUserVipStatus(db, userId);
  const isVipFree = hasVipAccess(vipSnapshot);
  return {
    cost: isVipFree ? 0 : baseCost,
    isVipFree,
  };
}
...
router.post('/generate-from-image', auth, async (req, res, next) => {
  try {
    ...
    const usageMeta = await getAiUsageMeta(userId, IMAGE_AI_COST);
    await deductCoins(userId, usageMeta.cost);
...
router.post('/custom', auth, async (req, res, next) => {
  try {
    ...
    const usageMeta = await getAiUsageMeta(userId, TEXT_AI_COST);
    await deductCoins(userId, usageMeta.cost);
