...
const { syncUserVipStatus } = require('../services/vipService');
...
const getProfile = async (req, res) => {
  try {
    const [users] = await pool.query(
      `SELECT id, uid, username, email, identity, avatar, vip_level, vip_plan_key, vip_expires_at, coins,
    ...
    const user = users[0];
    const vipSnapshot = await syncUserVipStatus(pool, req.user.id);
    ...
    return success(res, {
      ...
      avatar: user.avatar,
      vipLevel: vipSnapshot?.vipLevel || user.vip_level,
      vipPlanKey: vipSnapshot?.vipPlanKey || user.vip_plan_key || null,
      vipExpiresAt: vipSnapshot?.vipExpiresAt || user.vip_expires_at || null,
      coins: user.coins,
      gender: user.gender,
