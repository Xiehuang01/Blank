...
const { hasVipAccess, syncUserVipStatus } = require('../services/vipService');

let ensureDeleteColumnsPromise = null;
let ensureScheduleColumnsPromise = null;
...
const ensurePostcardScheduleColumns = async () => {
  if (ensureScheduleColumnsPromise) return ensureScheduleColumnsPromise;

  ensureScheduleColumnsPromise = (async () => {
    const [scheduledAtColumns] = await pool.query("SHOW COLUMNS FROM postcards LIKE 'scheduled_at'");
    if (scheduledAtColumns.length === 0) {
      await pool.query("ALTER TABLE postcards ADD COLUMN scheduled_at DATETIME DEFAULT NULL COMMENT '定时发送时间'");
    }
  })().catch((err) => {
...
const isRecipientFriendOrSelf = async (connection, senderId, recipientId) => {
  if (!recipientId) return false;
  if (Number(senderId) === Number(recipientId)) return true;
...
const processScheduledPostcards = async () => {
  await ensurePostcardScheduleColumns();
  ...
  for (const row of rows) {
    const [result] = await pool.query(
      `UPDATE postcards
       SET status = 'reviewing', review_reason = NULL
       WHERE id = ? AND status = 'scheduled'`,
      [row.id]
    );

    if (result.affectedRows > 0) {
      moderatePostcard(row.id).catch((err) => {
        console.error('Scheduled moderation error for postcard', row.id, err);
      });
    }
  }
};
...
const startScheduledPostcardProcessor = () => {
  if (scheduledProcessorStarted) return;
  scheduledProcessorStarted = true;

  const run = async () => {
    try {
      await processScheduledPostcards();
    } catch (err) {
      console.error('处理定时发送明信片失败:', err);
    }
  };
...
const normalizeElementForCompare = (el = {}) => {
  ...
  return {
    ...
    fontSize: Number(el.fontSize || 0),
    fontWeight: el.fontWeight || '',
    fontStyle: el.fontStyle || '',
    color: el.color || '',
    textDecoration: el.textDecoration || '',
    textAlign: el.textAlign || '',
    creatorId: Number(el.creatorId || 0),
    creatorName: el.creatorName || '',
  };
};

const isElementEqual = (a, b) => JSON.stringify(normalizeElementForCompare(a)) === JSON.stringify(normalizeElementForCompare(b));
...
const create = async (req, res) => {
  ...
  try {
    const {
      ...
      stampId, recipientInput, isPublic, scheduledAt,
    } = req.body;
    ...
    const vipSnapshot = await syncUserVipStatus(connection, req.user.id, { forUpdate: true });
    const isVipUser = hasVipAccess(vipSnapshot);
    ...
    const normalizedPostcardType = postcardType || 'normal';
    const shouldValidateRecipient = normalizedPostcardType === 'normal';
    ...
    if (shouldValidateRecipient) {
      ...
      const [recipientUsers] = await connection.query(
        'SELECT id, uid, email FROM users WHERE uid = ? OR email = ? LIMIT 1',
      ...
      normalizedRecipientInput = recipientUser.uid;
    }

    const parsedScheduledAt = parseScheduledAt(scheduledAt);
    const shouldSchedule = shouldValidateRecipient && Boolean(scheduledAt);

    if (shouldSchedule) {
      if (!parsedScheduledAt) {
        await connection.rollback();
        return error(res, '定时发送时间无效', 400);
      }

      if (parsedScheduledAt.getTime() <= Date.now()) {
        await connection.rollback();
        return error(res, '定时发送时间必须晚于当前时间', 400);
      }

      const recipientAllowed = await isRecipientFriendOrSelf(connection, req.user.id, recipientUser?.id);
      if (!recipientAllowed) {
        await connection.rollback();
        return error(res, '定时发送仅支持发送给好友或自己', 400);
      }
    }

    const statusToSave = shouldSchedule ? 'scheduled' : 'reviewing';
    const formattedScheduledAt = shouldSchedule ? formatMysqlDatetime(parsedScheduledAt) : null;
    ...
    const [result] = await connection.query(
      `INSERT INTO postcards
       (user_id, title, image_url, aspect_ratio, postcard_type,
        ...
        stamp_id, recipient_input, is_public, status, scheduled_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        req.user.id,
        title || '',
    ...
    return success(res, {
      id: postcardId,
      remainingStampQuantity: isVipUser ? currentQuantity : currentQuantity - 1,
      status: statusToSave,
      scheduledAt: formattedScheduledAt,
      stampConsumed: !isVipUser,
      vipFreePostage: isVipUser,
    }, shouldSchedule ? '明信片已预约发送' : '明信片发送成功，正在审核中', 201);
  } catch (err) {
    if (connection) {
      await connection.rollback();
    }
...
/**
 * 保存漂流元素（用于离开页面时保存位置/样式编辑）
 * PUT /api/postcards/:id/drift-elements
 */
const updateDriftElements = async (req, res) => {
  try {
    const { id } = req.params;
    const { elements } = req.body;

    if (!Array.isArray(elements)) {
      return error(res, '请提供有效的元素列表', 400);
    }
    ...
    const originElements = typeof rows[0].elements === 'string'
      ? JSON.parse(rows[0].elements || '[]')
      : (rows[0].elements || []);
    ...
    if (!isOwner) {
      if (elements.length !== originElements.length) {
        return error(res, '仅支持保存自身元素编辑，不支持新增或删除', 400);
      }

      for (let i = 0; i < originElements.length; i += 1) {
        const oldEl = originElements[i];
        const newEl = elements[i];

        if (!newEl || typeof newEl !== 'object') {
          return error(res, '元素数据格式错误', 400);
        }

        if (oldEl?.creatorId !== req.user.id && !isElementEqual(oldEl, newEl)) {
          return error(res, '无权限修改其他人的漂流元素', 403);
        }

        if (oldEl?.creatorId === req.user.id) {
          newEl.creatorId = oldEl.creatorId;
          newEl.creatorName = oldEl.creatorName;
    ...
    return success(res, { elements }, '漂流元素保存成功');
  } catch (err) {
    console.error('保存漂流元素错误:', err);
...
module.exports = {
  ...
  deleteDriftElement,
  updateDriftElements,
  startScheduledPostcardProcessor,
};
